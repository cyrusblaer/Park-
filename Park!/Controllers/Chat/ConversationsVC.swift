...
class ConversationsVC: UIViewController, UITableViewDelegate, UITableViewDataSource {
    ...
    func customization()  {
        ...
        //right bar button
        let icon = UIImage.init(named: "close_white")?.withRenderingMode(.alwaysOriginal)
        let rightButton = UIBarButtonItem.init(title: "返回", style: .plain, target: self, action: #selector(ConversationsVC.showContacts))
...
//        }
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        default:
 */
            let cell = tableView.dequeueReusableCell(withIdentifier: "conversationCell", for: indexPath) as! ConversationsTBCell
            cell.clearCellData()
//            cell.profilePic.image = self.items[indexPath.row].user.profilePic
            cell.nameLabel.text = "客服"
//                self.items[indexPath.row].user.name
        let message = "Hello"
        //                    self.items[indexPath.row].lastMessage.content as! String
...
//                cell.messageLabel.text = message
//            case .location:
//                cell.messageLabel.text = "[位置]"
//            default:
//                cell.messageLabel.text = "[图片]"
//            }
            let messageDate = Date()
//                .init(timeIntervalSince1970: TimeInterval(self.items[indexPath.row].lastMessage.timestamp))
